...
def get_text(path):
    for i in range(1, 6):
        ...
        file_path = path.replace("Protein_Data.xlsx", file_name)
        print(file_path)
...
def train_model():
    ...
    get_text(r'files/Protein_Data.xlsx')
    print("train cbow models 1 .. Please wait patiently")
    sentences = word2vec.Text8Corpus(r'files/all_1.txt')  # 将语料保存在sentence中
    word2vec.Word2Vec(sentences=sentences, min_count=0, size=500, window=5, compute_loss=True, iter=10, workers=8, sg=0, callbacks=[EpochLogger("l1")]) ## 生成词向量空间模型
    print("train cbow models 2 .. Please wait patiently")
    sentences = word2vec.Text8Corpus(r'files/all_2.txt')
    word2vec.Word2Vec(sentences=sentences, min_count=0, size=500, window=5, compute_loss=True, iter=10, workers=8, sg=0, callbacks=[EpochLogger("l2")])
    print("train cbow models 3 .. Please wait patiently")
    sentences = word2vec.Text8Corpus(r'files/all_3.txt')
    word2vec.Word2Vec(sentences=sentences, min_count=0, size=500, window=5, compute_loss=True, iter=10, workers=8, sg=0, callbacks=[EpochLogger("l3")])
    print("train cbow models 4 .. Please wait patiently")
    sentences = word2vec.Text8Corpus(r'files/all_4.txt')
    word2vec.Word2Vec(sentences=sentences, min_count=0, size=500, window=5, compute_loss=True, iter=10, workers=8, sg=0, callbacks=[EpochLogger("l4")])
    print("train cbow models 5 .. Please wait patiently")
    sentences = word2vec.Text8Corpus(r'files/all_5.txt')
    word2vec.Word2Vec(sentences=sentences, min_count=0, size=500, window=5, compute_loss=True, iter=10, workers=8, sg=0, callbacks=[EpochLogger("l5")])
...
def get_features(file_path, model):
    ...
    count = 0
    features = None
    for line in tqdm(content): #tqdm是一个显示进度条的python工具包
        line = line.split()
...
def merge_features():
    ...
    np.save(r'files/CBOW_features_Web.npy', CBOW_features)
...
if __name__ == '__main__':
    path = r'files/Protein_Data.xlsx'
    # get_text(path)
    merge_features()
