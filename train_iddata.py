...
if __name__ == '__main__':
    ...
    ## level0.append(('GBDT',GradientBoostingClassifier(loss='deviance', learning_rate=0.0798, n_estimators=369,
    ##                                   subsample=0.802, criterion='friedman_mse', min_samples_split=180
    ##                                   , min_samples_leaf=135,min_weight_fraction_leaf=0.140
    ##                                   , max_depth=93, min_impurity_decrease=0.0
   # #                                   , init=None, random_state=None, max_features=68, verbose=0
    ##                                   , max_leaf_nodes=None, warm_start=False
    ##                                   , validation_fraction=0.1
   # #                                   , n_iter_no_change=None, tol=0.01428)))

    ## level0.append(('GBDT', GradientBoostingClassifier(max_depth=19, min_samples_split=300, min_samples_leaf=10,
   ...
   # # level0.append(
    ...
    # clf8 = KMeans(n_clusters=2, max_iter=20, n_init=15, random_state=30)
    # 参数寻优：
    ...
    Voting = VotingClassifier(
        estimators=[('Logistic',grid_Logistic),('knn', grid_KNeighbors),('svm', level2),('RandomForestClassifier', grid_Forest), ('MLPClassifier', grid_MLP),('AdaBoost', grid_AdaBoost)],  # ,,('GBDT',best_GBDT),('svm', best_SVC)
        voting='hard')
    for clf, label in zip(
            [clf1,grid_Logistic,grid_KNeighbors,level2,grid_Forest, grid_MLP],   #    grid_AdaBoost,grid_XGB, grid_DecisionTree, grid_GaussianNB,, grid_SVM,
        ...
        for train_index, test_index in kf.split(x_train):
            ...
            AUC.append(auc)
            PRC.append(prc)
            joblib.dump(clf, './model/kmer/' + str(label) + '/' + str(k) + 'predict_protein.pkl')
            k = k + 1
